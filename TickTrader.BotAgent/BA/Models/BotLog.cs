...
namespace TickTrader.BotAgent.BA.Models
{
    public class BotLog : Actor
    {
        ...
        public class Handler : BlockingHandler<BotLog>, IBotLog
        {
            ...
            public Stream GetFileReadStream(string file) => CallActor(a => a.GetFileReadStream(file));
            public Stream GetFileWriteStream(string file) => throw new NotSupportedException("Writing files in bot logs folder is not allowed");
        }
        ...
        private Stream GetFileReadStream(string file)
        {
            if (file.IsFileNameValid())
            {
                var fullPath = Path.Combine(_logDirectory, file);
