...
//--- indicator buffers mapping
   ...
   bids_file = open_csv_file(StringFormat("bids_%s_%s_%d_%d_%d.bin", symbol, timeframe, depth, deviation, backstep));
   indicator_files_total = 1*1;
   for (int i = 0; i < 1; i++)
      for (int j = 0; j < 1; j++)
      {
         int ind = 1*i+j;
         indicator_params[ind][0] = depth;
         indicator_params[ind][1] = deviation;
         indicator_params[ind][2] = backstep;
         indicator_files[ind] = open_csv_file(StringFormat("%s_%d_%d_%d.bin", indicator_base, depth, deviation, backstep));
                ...
                const int &spread[])
  {
   ...
   for (int i = rates_total - 1; i >= prev_calculated; i--)
   {
      ...
      for (int j = 0; j < indicator_files_total; j++)
         for (int k = 0; k < 1; k++)
         {
            double val = iCustom(NULL, 0, "Zag", indicator_params[j][0], indicator_params[j][1], indicator_params[j][2], k, i);
