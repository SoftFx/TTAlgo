...
//--- indicator buffers mapping
   ...
   for (int i = 0; i < 4; i++)
      for (int j = 0; j < 7; j++)
      {
         ...
         indicator_files[ind] = open_csv_file(StringFormat("%s_%d_%d_%d.bin", indicator_base, period, i, j));
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   for (int i = rates_total - 1; i >= prev_calculated; i--)
   {
      ...
      {
         double val = iForce(NULL, 0, indicator_params[j][0], indicator_params[j][1],
            indicator_params[j][2], i);
