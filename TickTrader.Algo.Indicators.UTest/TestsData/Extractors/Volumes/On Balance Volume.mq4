...
//--- indicator buffers mapping
   ...
   for (int i = 0; i < 1; i++)
      ...
      {
         int ind = 7*i+j;
         indicator_params[ind][0] = j;
         indicator_files[ind] = open_csv_file(StringFormat("%s_%d.bin", indicator_base, j));
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   for (int i = rates_total - 1; i >= prev_calculated; i--)
   {
      ...
      {
         double val = iOBV(NULL, 0, indicator_params[j][0], i);
