...
//--- indicator buffers mapping
   ...
   bids_file = open_csv_file(StringFormat("bids_%s_%s_%d_%d.bin", symbol, timeframe, period, shift));
   ...
   for (int i = 0; i < 4; i++)
      for (int j = 0; j < 7; j++)
      {
         ...
         indicator_params[ind][2] = i;
         indicator_params[ind][3] = j;
         indicator_files[ind] = open_csv_file(StringFormat("%s_%d_%d_%d_%d.bin", indicator_base, period, shift, i, j));
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   for (int i = rates_total - 1; i >= prev_calculated; i--)
   {
      ...
      {
         double val = iStdDev(NULL, 0, indicator_params[j][0], indicator_params[j][1],
