...
//--- indicator buffers mapping
   ...
   bids_file = open_csv_file(StringFormat("bids_%s_%s_%.3f_%.3f.bin", symbol, timeframe, step, maximum));
   indicator_files_total = 1*1;
   for (int i = 0; i < 1; i++)
      for (int j = 0; j < 1; j++)
      {
         int ind = 1*i+j;
         indicator_params[ind][0] = step;
         indicator_params[ind][1] = maximum;
         indicator_files[ind] = open_csv_file(StringFormat("%s_%.3f_%.3f.bin", indicator_base, step, maximum));
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   for (int i = rates_total - 1; i >= prev_calculated; i--)
   {
      ...
      {
         double val = iSAR(NULL, 0, indicator_params[j][0], indicator_params[j][1], i);
