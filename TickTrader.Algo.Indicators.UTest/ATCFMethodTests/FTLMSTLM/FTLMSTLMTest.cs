...
{
    [TestClass]
    public class FtlmStlmTest : TestBase
    {
        private void TestMeasures(string symbol, string timeframe)
        {
            var dir = PathHelper.MeasuresDir("ATCFMethod", "FTLMSTLM");
            var test = new FtlmStlmTestCase(
                typeof(ATCFMethod.FTLMSTLM.FtlmStlm), symbol,
                $"{dir}bids_{symbol}_{timeframe}.bin", $"{dir}FTLMSTLM_{symbol}_{timeframe}");
            LaunchTestCase(test);
        }
