...
{
    [TestClass]
    public class OnBalanceVolumeTest : TestBase
    {
        private void TestMeasures(string symbol, string timeframe)
        {
            var dir = PathHelper.MeasuresDir("Volumes", "OnBalanceVolume");
            var test = new OnBalanceVolumeTestCase(typeof (Indicators.Volumes.OnBalanceVolume.OnBalanceVolume), symbol,
                $"{dir}bids_{symbol}_{timeframe}.bin", $"{dir}OBV_{symbol}_{timeframe}");
            LaunchTestCase(test);
        }
