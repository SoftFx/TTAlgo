...
namespace TickTrader.Algo.Core
{
    internal class TradeEmulator : TradeCommands, IExecutorFixture
    {
                ...
                try
                {
                    using (calc.UsageScope())
                    {
                        ...
                        using (JournalScope())
                        {
                            ...
                            var order = OpenOrder(calc, request.Type.ToCoreEnum(), request.Side.ToCoreEnum(), volume, maxVisibleVolume, price, stopPrice, sl, tp, request.Comment, request.Options, request.Tag, request.Expiration, OpenOrderOptions.None);
        ...
        public Task<OrderCmdResult> ModifyOrder(bool isAysnc, Api.ModifyOrderRequest request)
        {
            return ExecTradeRequest(isAysnc, async () =>
            {
                ...
                try
                {
                    ...
                    var orderVolume = orderToModify.Entity.RemainingVolume;
        ...
        Task<OrderCmdResult> TradeCommands.CloseOrder(bool isAysnc, CloseOrderRequest request)
        {
            var req = new CloseOrderCoreRequest
            {
                OrderId = request.OrderId,
        ...
        private Task<OrderCmdResult> CloseOrder(bool isAysnc, CloseOrderCoreRequest request)
        {
            return ExecTradeRequest(isAysnc, async () =>
            {
                OrderCmdResultCodes error = OrderCmdResultCodes.UnknownError;
        ...
        private OrderAccessor ConfirmOrderOpening(OrderAccessor order, BO.TradeTransReasons trReason, double? execPrice, decimal? execAmount, OpenOrderOptions options)
        {
            ...
            else if (order.Type == Domain.OrderInfo.Types.Type.Limit && order.HasOption(OrderExecOptions.ImmediateOrCancel))
            {
                // fill order
                fillInfo = FillOrder(order, execPrice, execAmount, trReason);
                //else
        ...
        private OrderAccessor ReplaceOrder(ReplaceOrderCoreRequest request)
        {
            // Check schedule for the symbol
        ...
        private OrderAccessor ConfirmOrderReplace(OrderAccessor order, ReplaceOrderCoreRequest request)
        {
            ...
            var newVol = (decimal?)request.NewVolume;

            if (order.IsPending && newVol.HasValue && newVol != order.Amount)
            {
                var filledVolume = order.Amount - order.RemainingAmount;

                order.Amount = newVol.Value;
            ...
            if (order.IsPending && request.MaxVisibleVolume.HasValue)
            {
                ...
                {
                    order.Entity.MaxVisibleVolume = (decimal)request.MaxVisibleVolume;
        ...
        private FillInfo FillOrder(OrderAccessor order, double? fillPrice, decimal? fillAmount, BO.TradeTransReasons reason)
        {
            double actualPrice;
        ...
        private FillInfo FillOrder2(OrderAccessor order, double fillPrice, decimal fillAmount, BO.TradeTransReasons reason)
        {
            if (order.Type == Domain.OrderInfo.Types.Type.Position)
        ...
        private OrderAccessor ConfirmOrderCancelation(BO.TradeTransReasons trReason, OrderAccessor order, OrderAccessor originalOrder = null)
        {
            ...
            report.Entity.MaxVisibleQuantity = (double?)order.Entity.MaxVisibleVolume;
        ...
        private OrderAccessor CreatePosition(BO.TradeTransReasons trReason, OrderAccessor parentOrder, Domain.OrderInfo.Types.Side side, SymbolAccessor smb, double openPrice, decimal posAmount, bool transformOrder)
        {
            ...
            var wasTmpOrder = parentOrder.Type == Domain.OrderInfo.Types.Type.Market
                || (parentOrder.Type == Domain.OrderInfo.Types.Type.Limit && parentOrder.HasOption(OrderExecOptions.ImmediateOrCancel));
            ...
            position.InitialType = (parentOrder != null) ? parentOrder.InitialType : Domain.OrderInfo.Types.Type.Market;
        ...
        private void ActivateOrder(ActivationRecord record, ref double lockedActivateMargin)
        {
            ...
            if (record.ActivationType == ActivationType.Pending)
            {
                ...
                {
                    fillInfo = FillOrder(record.Order, record.ActivationPrice, (decimal)record.Order.RemainingAmount, BO.TradeTransReasons.PndOrdAct);
        ...
        private void ActivateStopLimitOrder(OrderAccessor order, BO.TradeTransReasons reason)
        {
            ...
            OpenOrder(order.Calculator, Domain.OrderInfo.Types.Type.Limit, order.Side, order.Entity.RemainingVolume, null, order.Price,
            ...
            decimal? amount, double? price, SymbolAccessor smb, ClosePositionOptions options, string posById = null)
        {
            ...
            if (partialClose)
            {
                ...
                {
                    var partialSwap = CommisionEmulator.GetPartialSwap(position.Swap.Value, k, _calcFixture.RoundingDigits);
        ...
        private void ValidateTypeAndPrice(OrderType orderType, double? price, double? stopPrice, double? sl, double? tp, decimal? maxVisibleVolume, OrderExecOptions options, SymbolAccessor symbol)
        {
