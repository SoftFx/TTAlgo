...
{
    public class FxStreetRss : BaseCalendar<FxStreetNewsModel, FxStreetFilter>
    {
        private string _rss = @"http://xml.fxstreet.com/fundamental/economic-calendar/events.xml";

        public FxStreetRss(FxStreetFilter filter) : base(filter)
        {
        }
        ...
        private IEnumerable<FxStreetNewsModel> Parse(string data)
        {
            ...
            var feeds = feedXml.Descendants("item").Select(feed =>
            {
                ...
                return new FxStreetNewsModel("Rss", linkElement?.Value ?? "", "")
                {
                    DateUtc = date,
                    Event = evnt,
                    Actual = actual,
