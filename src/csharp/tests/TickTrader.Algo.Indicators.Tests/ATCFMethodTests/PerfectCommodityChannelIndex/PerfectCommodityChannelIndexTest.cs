...
{
    [TestClass]
    public class PerfectCommodityChannelIndexTest : TestBase
    {
        private void TestMeasures(string symbol, string timeframe)
        {
            var dir = PathHelper.MeasuresDir("ATCFMethod", "PerfectCommodityChannelIndex");
            var test = new PerfectCommodityChannelIndexTestCase(
                typeof(ATCFMethod.PerfectCommodityChannelIndex.PerfectCommodityChannelIndex), symbol,
                $"{dir}bids_{symbol}_{timeframe}.bin", $"{dir}PCCI_{symbol}_{timeframe}");
            LaunchTestCase(test);
        }
