...
namespace TickTrader.Algo.Indicators.Tests.ATCFMethodTests.ReferenceFastTrendLine
{
    [TestClass]
    public class ReferenceFastTrendLineTest : TestBase
    {
        private void TestMeasures(string symbol, string timeframe)
        {
            var dir = PathHelper.MeasuresDir("ATCFMethod", "ReferenceFastTrendLine");
            var test = new ReferenceFastTrendLineTestCase(
                typeof(ATCFMethod.ReferenceFastTrendLine.ReferenceFastTrendLine), symbol,
                $"{dir}bids_{symbol}_{timeframe}.bin", $"{dir}RFTL_{symbol}_{timeframe}");
            LaunchTestCase(test);
        }
