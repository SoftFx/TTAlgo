...
namespace TickTrader.Algo.Indicators.Tests.ATCFMethodTests.FastTrendLineMomentum
{
    [TestClass]
    public class FastTrendLineMomentumTest : TestBase
    {
        private void TestMeasures(string symbol, string timeframe)
        {
            var dir = PathHelper.MeasuresDir("ATCFMethod", "FastTrendLineMomentum");
            var test = new FastTrendLineMomentumTestCase(
                typeof(ATCFMethod.FastTrendLineMomentum.FastTrendLineMomentum), symbol,
                $"{dir}bids_{symbol}_{timeframe}.bin", $"{dir}FTLM_{symbol}_{timeframe}");
            LaunchTestCase(test);
        }
