...
//--- indicator buffers mapping
   ...
   bids_file = open_csv_file(StringFormat("bids_%s_%s_%d_%d_%d.bin", symbol, timeframe, tenkan, kijun, senkou));
   indicator_files_total = 1*1;
   for (int i = 0; i < 1; i++)
      for (int j = 0; j < 1; j++)
      {
         int ind = 1*i+j;
         indicator_params[ind][0] = tenkan;
         indicator_params[ind][1] = kijun;
         indicator_params[ind][2] = senkou;
         indicator_files[ind] = open_csv_file(StringFormat("%s_%d_%d_%d.bin", indicator_base, tenkan, kijun, senkou));
                ...
                const int &spread[])
  {
   ...
   for (int i = rates_total - 1; i >= prev_calculated; i--)
   {
      ...
      for (int j = 0; j < indicator_files_total; j++)
         for (int k = 1; k < 6; k++)
         {
            double val = iIchimoku(NULL, 0, indicator_params[j][0], indicator_params[j][1], indicator_params[j][2], k, i);
