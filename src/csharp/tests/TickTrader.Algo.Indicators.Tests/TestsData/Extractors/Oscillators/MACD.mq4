...
//--- indicator buffers mapping
   ...
   bids_file = open_csv_file(StringFormat("bids_%s_%s_%d_%d_%d.bin", symbol, timeframe, fastEma, slowEma, macdSma));
   indicator_files_total = 1*7;
   for (int i = 0; i < 1; i++)
      for (int j = 0; j < 7; j++)
      {
         int ind = 7*i+j;
         indicator_params[ind][0] = fastEma;
         indicator_params[ind][1] = slowEma;
         indicator_params[ind][2] = macdSma;
         indicator_params[ind][3] = j;
         indicator_files[ind] = open_csv_file(StringFormat("%s_%d_%d_%d_%d.bin", indicator_base, fastEma, slowEma, macdSma, j));
                ...
                const int &spread[])
  {
   ...
   for (int i = rates_total - 1; i >= prev_calculated; i--)
   {
      ...
      for (int j = 0; j < indicator_files_total; j++)
         for (int k = 0; k < 2; k++)
         {
            double val = iMACD(NULL, 0, indicator_params[j][0], indicator_params[j][1],
                  indicator_params[j][2], indicator_params[j][3], k, i);
